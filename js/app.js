...
(function (global) {
  ...
  const { refreshTable } = global.CoreGrist;
  let gristApi = null;
  let appReady = false;
  ...
  async function initializeApp() {
    ...
    bootPromise = (async () => {
      ...
      const tables = await global.CoreGrist.loadAllTables();
      Object.assign(state.tables, tables);
      appReady = true;
      debugLog('[App] Grist ready; tables loaded; rendering.');
      renderProjectsList();
    })().catch(error => {
      appReady = false;
      debugError('Initialisation Grist/app', error);
      if (typeof showToast === 'function') showToast('Erreur d’initialisation : ' + error.message, true);
      throw error;
    });
    return bootPromise;
  }
  ...
  function requireReady() {
    if (!appReady || !gristApi) {
      if (typeof showToast === 'function') showToast('L’application est encore en cours d’initialisation.', true);
      return false;
    }
    return true;
  }

  // Expose only a promise/status hook; no second grist.ready() call is permitted.
  global.AppLifecycle = { initializeApp, isReady: () => appReady, requireReady };
  initializeApp().catch(() => {});
