(function (global) {
  ...
  function openPosteModal({ mode = 'create', poste = null, initialName = '', originInput = null } = {}) {
    ...
    const refreshEmployeurs = () => { const structureRow = rows('Structures').find(row => Number(row.id) === Number(structure.select.value)); const allowed = ids(structureRow?.Toutes_les_tutelles); const all = rows('Etablissements'); const filtered = allowed.length ? all.filter(row => allowed.some(id => Number(id) === Number(row.id))) : []; employeur.render(filtered, structureRow?.Etablissement_Tutuelle_gestionaire); };
    structure.select.addEventListener('change', refreshEmployeurs); refreshEmployeurs();
