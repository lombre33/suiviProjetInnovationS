(function (global) {
  ...
  function openPosteModal({ mode = 'create', poste = null, initialName = '', originInput = null } = {}) {
    ...
    const refreshPreview = () => {
      ...
      preview.value = calculatedPosteName({ titre: modal.querySelector('#cpp-titre').value, precisions: modal.querySelector('#cpp-precisions').value, structureRow, employeurRow, id: editing ? poste?.id : estimatedId });
    };
    const refreshEmployeurs = () => { const structureRow = rows('Structures').find(row => Number(row.id) === Number(structure.select.value)); const allowed = ids(structureRow?.Toutes_les_tutelles); const all = rows('Etablissements'); const primaryId = refId(structureRow?.Etablissement_Tutuelle_gestionaire); const filtered = allowed.length ? all.filter(row => allowed.some(id => Number(id) === Number(row.id)) || Number(row.id) === Number(primaryId)) : []; employeur.render(filtered, primaryId); refreshPreview(); };
    ...
    if (!editing) { nextPosteId().then(id => { estimatedId = id; refreshPreview(); }).catch(() => {}); }
